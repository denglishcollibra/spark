...
#==============================================================================
: "${SPARK_VERSION:?ERROR: SPARK_VERSION is not set (e.g., 3.5.5)}"
...
: "${DOCKER_FILE:-?ERROR: DOCKER_FILE is not set (e.g., K8sJammy17.dockerfile)}"
PROJ_HOME="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
...
SPARK_DOCKER_SUBDIR="master/${SPARK_VERSION}/scala2.12-java17-ubuntu" # Common subdirectory for Dockerfile and entrypoint

# Location of your Dockerfile template (create this file as described above)
DOCKERFILE_TEMPLATE_PATH="${PROJ_HOME}/$DOCKER_FILE"
...
# Body
#==============================================================================
banner "Starting Spark Custom Image Build for $SPARK_VERSION"
...
SPARK_OUT_FILE="$IMG_NAME-$SPARK_VERSION"
echo "SPARK_OUT_FILE.tgz: $SPARK_OUT_FILE.tgz"
if [ ! -f "spark-$SPARK_VERSION-bin-$SPARK_OUT_FILE.tgz" ]; then
  banner "Building custom Spark distribution..."
  ./dev/make-distribution.sh --name "$SPARK_OUT_FILE" \
  --tgz \
  -Pscala_2.12 \
  -Pkubernetes
else
  banner "Using existing Spark distribution: $SPARK_OUT_FILE.tgz"
fi
...
banner "Untarring Spark distribution..."
SPARK_DIST_TGZ="$PROJ_HOME/spark-$SPARK_VERSION-bin-$IMG_NAME-$SPARK_VERSION.tgz"
tar -xvf $SPARK_DIST_TGZ --strip-components=1 -C $DOCKER_BUILD_DIR
...
#  --build-arg SPARK_SUBDIR="spark-$SPARK_VERSION-bin-$IMG_NAME-$SPARK_VERSION" \
#  --build-arg SPARK_VERSION="$SPARK_VERSION" \
banner "Building Docker image..."
docker buildx build --platform linux/amd64,linux/arm64 \
  --build-arg SPARK_UID=1000 \
  --build-arg SPARK_TGZ_FILENAME="$SPARK_TGZ_FILENAME" \
  -t "$DOCKER_IMG_ROOT/$IMG_NAME:$SPARK_VERSION" \
